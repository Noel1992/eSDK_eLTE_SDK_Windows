...
struct _AudioStream
{
	...
	/* lijinglai add for audio recorder b */
	MSFilter *send_tee;
	MSFilter *recv_tee;
	MSFilter *recorder_mixer;
	MSFilter *recorder;
	char *recorder_file;
	/* lijinglai add for audio recorder e */
...
void audio_stream_change_writer(AudioStream *stream,MSSndCard *playcard);
void audio_stream_change_reader(AudioStream *stream,MSSndCard *captcard);
...
AudioStream *audio_stream_new_with_ip(const char* ip,int locport);  //add by wugang
...
RingStream *ring_start (const char *file, int interval, int duration, int cnt, MSSndCard *sndcard);
RingStream *ring_start_with_cb(const char *file,int interval,int duration, int cnt, MSSndCard *sndcard, MSFilterNotifyFunc func,void * user_data);
...
/* begin added by tanghailang for stat */
typedef struct
{
	uint32_t rtp_ts;
	uint64_t rtp_recv;
	uint64_t rtp_dequeue;
	uint64_t got_picture;
	uint64_t gui_enqueue;
}FrameTimeStat;
/* end added by tanghailang for stat */

VideoStream *video_stream_new(int locport, bool_t use_ipv6);
VideoStream *video_stream_new_with_ip(const char* ip,int locport); //add by wugang
...
/* begin added by tanghailang */
int video_stream_recv_only_start_ex(VideoStream * stream, RtpProfile *profile, const char *remip, int remport, int payload, int jitt_comp);
void video_stream_recv_only_stop_ex(VideoStream *stream);
/* end added by tanghailang */

/*send rtp probe packet for NAT,added by wangshiwei*/
mblk_t *rtp_session_create_nat_probe_packet(RtpSession *session, uint32_t SSRC);
