...
typedef struct rtp_extended_stats
{
  double rtt; /* round trip time (sec) extracted from RTCP reports */
	uint16_t djb_nominal_delay; /* De-jitter buffer nominal delay (ms) */
	uint16_t djb_max_delay; /* De-jitter buffer maximum delay (ms) */
  uint16_t djb_nominal_delay_hwmark; /* De-jitter buffer nominal delay high-water mark (ms) */
  uint16_t djb_max_delay_lwmark; /* De-jitter buffer nominal delay low-water mark (ms) */
  
} rtp_extended_stats_t;
