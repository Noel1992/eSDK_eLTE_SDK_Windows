	...
	if(!CeLTE_PlayerCtrl::m_ulBypass)
	{
		...
		if (NULL != pMediaMgr)
		{
			...
			if(m_pParentWnd)
			{
				...
				if(pCtrl)
				{
					pVideoRender = dynamic_cast<VideoRender*>(pMediaMgr->getRender(pCtrl->GetMediaPlayer().GetMediaPlayerResID()));
				}
			}
			if (NULL != pVideoRender)
			{
				pVideoRender->SetDstRect(0, 0, (int)(right - left), (int)(bottom - top));
			}
		}
	}
