...
public:
	...
	//激活关闭前后摄像头切换
	void EnableReversePlay(BOOL bEnable);
	// 设置伴音
	...
	// 获取OCX控件指针
	COleControl* GetCeLTE_PlayerCtrl(){return m_pCeLTE_PlayerCtrl;};
	// 获取播放窗口实例
	const CVideoStatic& GetVideoStatic() const {return m_VideoStatic;};
	// 获取语音按钮状态
	BOOL GetAudioBtnState() const;
