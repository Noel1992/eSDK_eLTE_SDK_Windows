...
int MediaPlayer::StartPlayer(IVS_CHAR* pRemoteIP, IVS_UINT32 uiRemoteVideoPort, IVS_UINT32 uiRemoteAudioPort, HWND hVideoPlayer)
{
	...
	{
		ELTE_INT32 iRet = ELTE_SDK_SetPlayWindow(m_strResId.c_str(), (ELTE_VOID*)hVideoPlayer);
...
int MediaPlayer::PlayAudio()
{
	...
	{
		// 构建静音参数, 视频回传
		std::string strParam ("<Content><MuteParam><CallType>2</CallType></MuteParam></Content>");
		// 执行静音
		iRet = ELTE_SDK_VolUnMute(m_strResId.c_str(), strParam.c_str());
		if (eLTE_ERR_SUCCESS != iRet)
		{
...
int MediaPlayer::StopAudio()
{
	...
	else
	{
		...
		// 执行静音
		iRet = ELTE_SDK_VolMute(m_strResId.c_str(), strParam.c_str());
		if (eLTE_ERR_SUCCESS != iRet)
		{
...
int MediaPlayer::LocalSnapshot(const std::string& strResId, std::string& strSnapshotPath)
{
	...
	{
		iRet = ELTE_SDK_LocalSnapshot(strResId.c_str(), strSnapshotPath.c_str(), (ELTE_ULONG)m_snapshotFormat);
		if (IVS_SUCCEED != iRet)
		{
			LOG_RUN_ERROR("ELTE_SDK_LocalSnapshot failed. (%d)", iRet);
			strSnapshotPath = "";
		}
	}
