...
void CVideoPane::OnSize(UINT nType, int cx, int cy)
{
	...
	if(!m_ulByMediaPass)
	{
		if (NULL != m_VideoStatic.GetSafeHwnd())
		{
			...
			{
				int iResId = ((CeLTE_PlayerCtrl*)m_pCeLTE_PlayerCtrl)->GetMediaPlayer().GetMediaPlayerResID();//lint !e1774
				...
				{
					LOG_RUN_ERROR("When change size, call ELTE_SDK_SetPlayWindowSize failed, the res id is %d.", iResId);
				}
			}
		}
	}
...
BOOL CVideoPane::GetAudioBtnState() const
{
	return m_AudioBtn.IsWindowEnabled();
}
...
void CVideoPane::EnableImageButton(BOOL bEnable)
{
	if (bEnable)
	{
		...
		if (NULL != m_ReversePlayBtn.GetSafeHwnd() && m_bEnableReversePlay)
		{
...
//激活关闭前后摄像头切换
void CVideoPane::EnableReversePlay(BOOL bEnable)
{
	m_bEnableReversePlay= bEnable;
	if (NULL != m_ReversePlayBtn.GetSafeHwnd())
	{
		CString strOcxPath = eLTE_Tool::GetOcxPath();
		strOcxPath.Append(IMG_RESOURCE_PATH);

		m_ReversePlayBtn.LoadImages(strOcxPath + IMG_REVERSEPLAY_NORMAL, strOcxPath + IMG_REVERSEPLAY_SELECT,
			strOcxPath + IMG_REVERSEPLAY_FOCUS, strOcxPath + IMG_REVERSEPLAY_DISABLE, ImgState_Disable);
		m_ReversePlayBtn.UpdateToolTip(m_Language.GetString(GET_STRING_PARAM(IDS_REVERSEPLAY)));
		m_ReversePlayBtn.Invalidate(TRUE);
	}
}
