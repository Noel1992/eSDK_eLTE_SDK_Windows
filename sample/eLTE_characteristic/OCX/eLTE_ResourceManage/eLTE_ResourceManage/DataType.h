...
typedef enum
{
	...
	eLTE_ERR_FAILED = 2135101001,			// 通用fail 
	eLTE_ERR_INVALID_PARAM,					// 非法入参
	eLTE_ERR_LOAD_TYPE,						// ocx load类型不支持
	eLTE_ERR_NOT_LOGIN,						// 没有登陆
	eLTE_ERR_WAIT_LOGIN_RESULT,				// 等待Login 结果中，或者没有Login success 

	//2135101201 - 2135101300 播放器错误
	eLTE_ERR_VIDEO_STATE = 2135101201,		// 视频播放 状态错误（如：正在播放中，不能播放）
	eLTE_ERR_AUDIO_OPEN,					// 音频播放 设备打开fail 
	eLTE_ERR_AUDIO_CLOSE,					// 音频播放 设备关闭fail 
	eLTE_ERR_AUDIO_RESET,					// 音频播放 设备重置fail 
	eLTE_ERR_AUDIO_PREPARE,					// 音频播放 预播声音数据fail 
	eLTE_ERR_AUDIO_UNPREPARE,				// 音频播放 清除预播声音数据fail 
	eLTE_ERR_AUDIO_WRITE,					// 音频播放 播发声音数据fail 
	eLTE_ERR_AUDIO_SET_VOLUME,				// 音频播放 设置音量fail 
	eLTE_ERR_AUDIO_GET_VOLUME,				// 音频播放 获取音量fail 
	eLTE_ERR_AUDIO_OUT_BUFFER,				// 音频播放 播放缓存已满	
	eLTE_ERR_AUDIO_DECODE,					// 音频播放 解码fail 

	//2135101301 - 2135101400 文件操作错误
	eLTE_ERR_CONFIG_FILE_LOAD = 2135101301,	// 加载ini配置文件fail 
	eLTE_ERR_CONFIG_FILE_READ,				// 读取ini配置文件fail 
	eLTE_ERR_CONFIG_FILE_WRITE,				// 写入ini配置文件fail 
	eLTE_ERR_LOG_FILE_OPEN,					// 打开/创建日志文件fail 

	//2135101401 - 2135101500 XML操作错误
	eLTE_ERR_XML_FIND_ELEM = 2135101401,	// 查找XML节点fail 
	eLTE_ERR_XML_GET_ELEM_VALUE				// 获取XML节点fail 
...
typedef enum
{
	...
	EVENT_NOTIFY_SDS_RETCODE                // 短信发送是否success 事件
} eLTE_EVENT_ID_E;
...
typedef enum
{
	...
	USERDGNASTATUS =21,      //user Dynamic reorganization状态
...
typedef enum {
	...
	STATUS_UNAUTHORIZED = 4017, //Without authentication 401
	...
	STATUS_GRPPATCH_CREATEOK = 4031,  //Patch Group创建OK
	STATUS_GRPPATCH_CREATEFAIL = 4032, //Patch Groupcreate failed
	STATUS_GRPPATCH_CANCELOK = 4033, //Patch Group取消OK
	STATUS_GRPPATCH_CANCELFAIL = 4034, //Patch Group取消fail 
	STATUS_GRPPATCH_ADDOK = 4035, //Patch Group添加member OK
	STATUS_GRPPATCH_ADDFAIL = 4036, //Patch Group添加member FAIL
	STATUS_GRPPATCH_DELOK = 4037, //Patch Groupdelete member OK
	STATUS_GRPPATCH_DELFAIL = 4038,  //Patch Groupdelete member FAIL
...
typedef enum {
	...
	/*------------媒体面状态-----------*/
	P2Pvideocall_IND_STATUS_ACKED_MEDIAPLANE_ERROR,  //视频RTP流建立fail 
	P2Pvideocall_IND_STATUS_ACKED_MEDIAPLANE_NOAUDIO, //视频伴音流建立fail 
...
typedef enum {
	...
	SDS_SEND_SUCCESS, //短信发送success 
	SDS_SEND_FAIL, //短信发送fail 
	...
	ATTACH_UPLOAD_FAIL,   //附件上传fail 
	ATTACH_DOWNLOAD_FAIL,  //附件下载fail 
	SIP_START_RUNNING,     //BCC 启动
	KICK_OFF,      //BCC 踢出调度台通知,因为有相同账号Login 
